...
def shortes_route(df_distance, adj_graph, starting_node, nodes_to_visit, pos):
    ...
    if not d_graph.are_nodes_connected(parts, {starting_node} | set(nodes_to_visit)):
        print("Not possible")
        return
...
def func_4():
    input_nodes = list(map(int, input('Write nodes you want to visit. First node will be starting node.').split()))
    data_type = input('Which distance to use (t,d,n)?')
    df_distance, adj_graph = load_data.choose_data(data_type)
    starting_node = input_nodes[0]
    node_to_visit = input_nodes[1:]
    shortes_route(df_distance, adj_graph, starting_node, set(node_to_visit), load_data.coordinates)
